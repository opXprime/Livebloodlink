<?php
// login — multi-step: email/pw → (admin: PIN + security Q) → (donor/hospital: security Q)
$pageTitle = 'Login';
...
// cleanup old failed attempts occasionally
if (rand(1, 20) === 1) {
    try { $db->exec("DELETE FROM login_attempts WHERE attempted_at < DATE_SUB(NOW(), INTERVAL 1 DAY)"); } catch (PDOException $ex) {}
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['step_credentials'])) {
    if (!verifyCSRF()) { $errors[] = 'Invalid token.'; }
        ...
        if (empty($email) || empty($password)) {
            ...
            $delCheck = $db->prepare("SELECT details FROM system_logs WHERE action='account_deleted_by_admin' AND details LIKE :e ORDER BY created_at DESC LIMIT 1");
            ...
            if ($delRecord) {
                ...
                if (!$user) {
                    ...
                    // load PIN from admin_credentials table
                    ...
                    // donor/hospital — go to security question
                    $_SESSION['login_pending_user'] = $user;
...
<div class="auth-container">
    <div class="card auth-card shadow">
        <div class="card-header"><h4 class="mb-0"><i class="fas fa-sign-in-alt me-2"></i>Login</h4></div>
        <div class="card-body p-4">
            ...
            <div id="deletedPopup" class="alert alert-danger shadow" style="position:fixed;top:80px;left:50%;transform:translateX(-50%);z-index:9999;max-width:420px;text-align:center;animation:slideIn .4s ease">
            ...
            <script>setTimeout(function(){var p=document.getElementById('deletedPopup');if(p){p.style.transition='opacity 0.5s';p.style.opacity='0';setTimeout(function(){p.remove();},500);}},5000);</script>
            <?php endif; ?>
            ...
            <form method="POST">
                ...
                <div class="text-center mt-3"><a href="login.php?reset=1" class="auth-link"><i class="fas fa-arrow-left me-1"></i>Start Over</a></div>
            </form>

            <?php elseif ($step === 'admin_pin'): ?>
            <?php $pu = $_SESSION['login_pending_user'] ?? []; ?>
            <div class="alert alert-warning"><i class="fas fa-key me-2"></i>Admin account detected. Enter your login PIN and security answer.</div>
            <form method="POST">
                <?= csrfField() ?>
                <div class="mb-3">
                    <label class="form-label fw-bold"><i class="fas fa-key me-1"></i>Login PIN</label>
                    <input type="password" class="form-control" name="login_pin" required autofocus maxlength="20" placeholder="Your personal login PIN">
                </div>
                <div class="mb-3">
                    <label class="form-label fw-bold"><?= e($pu['security_question'] ?? 'Security Question') ?></label>
