...
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['create_admin'])&&verifyCSRF()){
    ...
    $pwErrs=validatePassword($aPw); if($pwErrs) $errs=array_merge($errs,$pwErrs);
    ...
    if(empty($errs)){
        $chk=$db->prepare("SELECT id FROM users WHERE email=:e"); $chk->execute([':e'=>$aEmail]);
    ...
    if(!empty($errs)){
        ...
        $db->prepare("INSERT INTO users (name,email,password_hash,security_question,security_answer_hash,role,country_id,is_active) VALUES(:n,:e,:p,:sq,:sa,'admin',:c,1)")
            ->execute([':n'=>$aName,':e'=>$aEmail,':p'=>$hPw,':sq'=>$aSq,':sa'=>$hSa,':c'=>currentCountryId()]);
        $newAdminId=$db->lastInsertId();
        // store PIN separately in admin_credentials
...
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['update_password'])&&verifyCSRF()){
    ...
    if(!$row||!password_verify($curPw,$row['password_hash'])){
        ...
        if($pwErrs) setFlash('error',implode(' ',$pwErrs));
        elseif($newPw!==$confPw) setFlash('error','New passwords do not match.');
        else{
            $db->prepare("UPDATE users SET password_hash=:p WHERE id=:u")->execute([':p'=>password_hash($newPw,PASSWORD_BCRYPT,['cost'=>12]),':u'=>currentUserId()]);
...
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['update_pin'])&&verifyCSRF()){
    ...
    if(!$row||!password_verify($curPw,$row['password_hash'])){
        ...
        $db->prepare("UPDATE admin_credentials SET pin_hash=:p WHERE user_id=:u")->execute([':p'=>password_hash($newPin,PASSWORD_BCRYPT,['cost'=>12]),':u'=>currentUserId()]);
...
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['update_security_qa'])&&verifyCSRF()){
    ...
    if(!$row||!password_verify($curPw,$row['password_hash'])){
        ...
        $db->prepare("UPDATE users SET security_question=:q, security_answer_hash=:a WHERE id=:u")
            ->execute([':q'=>$newQ,':a'=>password_hash(strtolower($newA),PASSWORD_BCRYPT,['cost'=>12]),':u'=>currentUserId()]);
...
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['admin_delete_self'])&&verifyCSRF()){
    ...
    if($adminCount<=1){
        ...
        $db->prepare("DELETE FROM users WHERE id=:u")->execute([':u'=>currentUserId()]);
        session_destroy();
...
// load profile data for checklist
...
<!-- security checklist -->
<div class="card mb-4 border-info"><div class="card-header bg-info bg-opacity-10"><h5 class="mb-0 text-info"><i class="fas fa-clipboard-check me-2"></i>Security Checklist</h5></div>
<div class="card-body">
    <div class="row">
    <div class="col-md-6">
        <p class="mb-2"><?=$hasPin?'<i class="fas fa-check-circle text-success me-2"></i>':'<i class="fas fa-times-circle text-danger me-2"></i>'?>Login PIN is <?=$hasPin?'set':'<strong>not set</strong> — set one below'?></p>
        <p class="mb-2"><?=$hasSQ?'<i class="fas fa-check-circle text-success me-2"></i>':'<i class="fas fa-times-circle text-danger me-2"></i>'?>Security question is <?=$hasSQ?'set':'<strong>not set</strong>'?></p>
        <p class="mb-2"><?=!$pwOld?'<i class="fas fa-check-circle text-success me-2"></i>':'<i class="fas fa-exclamation-triangle text-warning me-2"></i>'?>Password <?=$pwOld?'may be stale (last update '.round($daysSinceUpdated).' days ago) — consider changing it':'recently updated'?></p>
    </div>
    <div class="col-md-6">
    ...
    <hr>
    <p class="text-muted small mb-0"><i class="fas fa-lightbulb me-1"></i>Change your password every few weeks. Avoid using your birthday or common words as your PIN. Pick a security answer only you would know — not something anyone could find on social media.</p>
</div></div>

<!-- change password / PIN / security Q -->
<div class="card mb-4"><div class="card-header bg-white"><h5 class="mb-0"><i class="fas fa-lock me-2"></i>Update Credentials</h5></div>
<div class="card-body">
    <div class="row g-4">
    <div class="col-md-4">
...
<!-- create new admin -->
<div class="card mb-4 border-primary"><div class="card-header bg-primary bg-opacity-10"><h6 class="mb-0 text-primary"><i class="fas fa-user-plus me-2"></i>Create New Admin</h6></div>
<div class="card-body">
...
<!-- delete own account -->
<div class="card border-danger"><div class="card-header bg-danger bg-opacity-10"><h5 class="mb-0 text-danger"><i class="fas fa-trash-alt me-2"></i>Delete My Admin Account</h5></div>
<div class="card-body">
...
<?=csrfField()?><div class="input-group"><input type="text" class="form-control" name="confirm_delete" placeholder="Type DELETE to confirm">
<button type="submit" name="admin_delete_self" class="btn btn-danger"><i class="fas fa-trash me-1"></i>Delete</button></div></form>
</div></div>
