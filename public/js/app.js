document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.alert-dismissible').forEach(function(a) {
        setTimeout(function(){ a.style.transition='opacity .5s'; a.style.opacity='0'; setTimeout(()=>a.remove(),500); }, 5000);
    });
    document.querySelectorAll('.btn-close').forEach(function(b) { b.onclick=function(){ this.closest('.alert').remove(); }; });
    ...
    if (tog) tog.onclick = function(){ document.getElementById('navbarContent').classList.toggle('show'); };
...
function initLocationCascade() {
    ...
    // Area autocomplete — trigger on input AND focus
    ...
    // Show all areas on focus (click into field) so user sees the list before typing
...
// password strength meter — checks length, uppercase, lowercase, numbers
